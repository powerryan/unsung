        ...
        <br></br>
            ...
            <div className="col-sm-11">
                <div className="fullpostInfo">
                    <span className="fullpostDate">{new Date(post.createdAt).toDateString()}</span>

                    <Link to={`/channel/${post.channel}`} className="link">
                    <span className="fullpostChannel">{post.channel+" Channel"}</span>
                    </Link>

                    <Link to={`/subchannel/${post.subchannel}`} className="link">
                    <span className="fullpostChannel">{post.subchannel+" Subchannel"}</span>
                    </Link>
                    ...
                    <span className="fullpostTitle">{post.title}</span>




                </div>
                <p className="fullpostDesc">
                {post.body}
                </p>
                {post.photo && (
                    <img
                        className="fullpostImg"
                        src={post.photo}
                        alt=""
                    />
                ...
                {followed ? <span className="fullpostChannel" onClick={unfollow}>Unfollow channel</span>:
                <span className="fullpostChannel" onClick={follow}>Follow channel</span> }
                {subscribed ? <span className="fullpostChannel" onClick={unsubscribe}>Unsubscribe</span>:
                <span className="fullpostChannel" onClick={subscribe}>Subscribe to poster</span> }
